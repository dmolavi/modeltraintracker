       ...
       $('#item_table td.control').live( 'click', function () {
		...
		$('#addMRForm').validate({
			...
			submitHandler: function(form) {
				...
				} else if(addMRVar == "roadname"){
					var addMRData = {ref:"roadname", roadname:$("#newentry").val()};
				} else if(addMRVar == "type"){
					var addMRData = {ref:"type", type:$("#newentry").val()};
				}
				$.ajax({
					type: "POST",
					url: "add.php",
		...
		$('#dialog-add-MR').dialog({
			...
			buttons: {
				...
				Cancel: function() {
					$( this ).dialog( "close" );
					$("#newentry").removeClass("error");
					$("#newtype").removeClass("error");
					$("div.addMRerror").hide();
				}
			},
			close: function() {
		...
		$("#newtype").change(function(){
			if ($("#newtype").val() == 'manufacturer') {
				addMRVar = "manufacturer";
				$("#togglediv").html("New Manufacturer: ");
			} else if ($("#newtype").val() == 'roadname') {
				addMRVar = "roadname";
				$("#togglediv").html("New Roadname: ");
			} else if ($("#newtype").val() == 'type') {
				addMRVar = "type";
				$("#togglediv").html("New Type: ");
			}
		});
		...
		$('#addManPopup,#addRoadPopup, #addTypePopup').live('click', function() {
...
?>
	<div id="content">
		...
		<div id="dialog-add-MR">
			...
			<form id="addMRForm" class="cmxform" action="#">
				<label for="newtype">What will you be adding?</label><br />
