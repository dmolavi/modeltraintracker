<?php
	...
	$query = "SELECT * FROM manufacturer m ORDER BY m.m_name";
	$res = mysql_query($query);
	$data = array();
	while($row = mysql_fetch_row($res)) {
		$data[]=$row;
	}	
	$colcount = ceil(count($data) / 25);
?>
...
<script type="text/javascript"> 
	$(function(){
		...
		oTable = $('#manufacturer_table').dataTable({
			"bFilter": false,
			"bJQueryUI": true,
			"iDisplayLength" : 25,
			"sDom": '<"H"lfr>t<"F"ip>',
			...
			sAddURL: "add.php",
			sSelectedRowClass: "",
			oAddNewRowButtonOptions: {	label: "Add new manufacturer",
										icons: {primary:'ui-icon-plus'}},
			oAddNewRowFormOptions: {	title: 'Add new manufacturer',
										width: 500,
										modal: true},
...
?>
	<div id="content">
		...
		<form id="formAddNewRow" action="#">
			...
			<input name="manufacturer" id="manufacturer" type="text" value="" size="15" maxlength="50" />
			<br />
			<input type="hidden" name="ref" value="manufacturer" />
		</form>	
		<div id="table_box">
		<table class="list datatable" id="manufacturer_table">
			<thead>
				<tr>
					<th>Manufacturer</th>
				</tr>
